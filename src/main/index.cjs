...
async function handleAnalyzeFiles(event, filePaths) {
  const results = [];
...
async function handleGenerateReports(event, { files, outputFolder, sortAlpha, convertNumbers, reportMonth, jobId, filesToDeduplicate = [], filesToSkip = [] }) {
  ...
  for (const corretora of corretoras) {
    ...
    for (const i of items) {
      const nome = i.info.isPrincipalCorretora ? i.info.corretoraGrupo : i.info.vendedor;
...
async function handleImportReadyReports(event, { files, reportMonth }) {
  ...
  for (const file of files) {
    try {
      ...
      const corretoraOriginal = getCorretoraFromFileName(file);
      const corretora = normalizarCorretoraParaGrupo(corretoraOriginal);
  ...
  for (const file of files) {
    try {
      ...
      for (let r = 1; r <= lastRow; r++) {
        ...
        let isHeader = text && 
          !text.startsWith('Lote:') && !text.startsWith('Período:') && 
          !text.startsWith('Total de') && !text.startsWith('Comissao_') &&
          (text.includes(' - ') || text.toUpperCase().includes('AS PRIME'));

        if (!isHeader && text) {
          const nextRowText = r < lastRow ? decodeHtml(getText(ws.getCell(r + 1, 1).value)) : '';
          if (nextRowText.startsWith('Período:')) {
            isHeader = true;
          }
        }

        if (isHeader) {
          const rawParsed = parseVendedorCorretora(cellVal, file);
          const parsed = {
            ...rawParsed,
            corretora: normalizarCorretoraParaGrupo(rawParsed.corretora)
          };
          ...
          if (parsed.corretora && parsed.corretora !== 'Corretora não identificada') {
            ...
            const key = `${file}::${parsed.corretora}::${defaultCategory}`;
            if (!blocksMap.has(key)) {
              ...
              if (blocksMap.get(key).total === 0 && total > 0) {
                blocksMap.get(key).total = total;
              }
            }
          }
        }
...
registerReportIpc({
  ...
  handlers: {
    ...
    analyzeFiles: handleAnalyzeFiles,
